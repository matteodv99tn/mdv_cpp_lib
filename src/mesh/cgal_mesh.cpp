...
mdv::mesh::construct_geodesic(
    ...
    std::vector<Mesh::CgalData::Point3> cgal_geod;
...
void
Mesh::CgalData::build_vertex_normals_map() noexcept {
...
//              |_|
Eigen::Vector3d
mdv::mesh::convert(const Mesh::CgalData::Vec3& x) {
    return {x.x(), x.y(), x.z()};
}

Eigen::Vector3d
mdv::mesh::convert(const Mesh::CgalData::Point3& x) {
    return {x.x(), x.y(), x.z()};
}
